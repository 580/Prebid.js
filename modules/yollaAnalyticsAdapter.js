...
function sendMessage(auctionId, bidWonId) {
  ...
  let message = {
    ...
    integration: config.getConfig('yolla.int_type') || DEFAULT_INTEGRATION,
    version: '$prebid.version$',
    referrerUri: referrer
  };
  const wrapperName = config.getConfig('yolla.wrapperName');
  if (wrapperName) {
    message.wrapperName = 0;
  }
  if (auctionCache && !auctionCache.sent) {
    let adUnitMap = Object.keys(auctionCache.bids).reduce((adUnits, bidId) => {
      ...
      // Add site and zone id if not there and if we found a yolla bidder
      if ((!adUnit.siteId || !adUnit.zoneId) && yollaAliases.indexOf(bid.bidder) !== -1) {
    ...
    // We need to mark each cached bid response with its appropriate yolla site-zone id
...
function getBidPrice(bid) {
  ...
  utils.logWarn('Yolla Analytics Adapter: Could not determine the bidPriceUSD of the bid ', bid);
}
...
let accountId;
// List of known yolla aliases
// This gets updated on auction init to account for any custom aliases present
let yollaAliases = ['yolla'];

/*
  Checks the alias registry for any entries of the yolla bid adapter.
  adds to the yollaAliases list if found
*/
function setYollaAliases(aliasRegistry) {
  Object.keys(aliasRegistry).forEach(function (alias) {
    if (aliasRegistry[alias] === 'yolla') {
      yollaAliases.push(alias);
    }
  });
}
...
let yollaAdapter = Object.assign({}, baseAdapter, {
  enableAnalytics(config = {}) {
    ...
    if (typeof config.options === 'object') {
      ...
      if (config.options.endpoint) {
        ...
        utils.logError('required endpoint missing from yolla analytics');
        error = true;
      }
      ...
      if (typeof config.options.samplingFactor !== 'undefined') {
        if (typeof config.options.sampling !== 'undefined') {
          utils.logWarn('Both options.samplingFactor and options.sampling enabled in yolla analytics, defaulting to samplingFactor');
        }
    ...
    if (validSamplingFactors.indexOf(samplingFactor) === -1) {
      error = true;
      utils.logError('invalid samplingFactor for yolla analytics: ' + samplingFactor + ', must be one of ' + validSamplingFactors.join(', '));
    } else if (!accountId) {
      error = true;
      utils.logError('required accountId missing for yolla analytics');
    }
  ...
  track({eventType, args}) {
    ...
    switch (eventType) {
      case AUCTION_INIT:
        // set the yolla aliases
        setYollaAliases(adapterManager.aliasRegistry);
        ...
        Object.assign(cache.auctions[args.auctionId].bids, args.bids.reduce((memo, bid) => {
          ...
          memo[bid.bidId] = utils.pick(bid, [
            ...
            'videoAdFormat', (_, cachedBid) => {
              console.log('cachedBid', cachedBid);
              if (cachedBid.bidder === 'yolla') {
                return ({
                  201: 'pre-roll',
                  202: 'interstitial',
                  203: 'outstream',
      ...
      case BID_RESPONSE:
        ...
        if (!bid) {
          utils.logError('Yolla Anlytics Adapter Error: Could not find associated bid request for bid response with requestId: ', args.requestId);
          break;
        }
        ...
        if (utils.deepAccess(bid, 'bidResponse.seatBidId') && bid.bidder === 'yolla' && bid.source === 'server' && ['video', 'banner'].some(i => utils.deepAccess(bid, 'bidResponse.mediaType') === i)) {
...
adapterManager.registerAnalyticsAdapter({
  adapter: yollaAdapter,
  code: 'yolla'
});

export default yollaAdapter;
