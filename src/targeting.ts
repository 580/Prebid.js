...
import {getYmpbOption} from './ympb.js'; // YMPB
import {getCpmStringValue} from './cpmBucketManager.ts'; // YMPB
...
export function newTargeting(auctionManager) {
  ...
  const targeting = {
    ...
    getAllTargeting(adUnitCode?: AdUnitCode | AdUnitCode[], bidLimit?: number, bidsReceived?: Bid[], winReducer = getHighestCpm, winSorter = sortByHighestCpm): ByAdUnit<TargetingValues> {
      ...
      const adUnitCodes = getAdUnitCodes(adUnitCode);
      // const adUnitBidLimit = getAdUnitBidLimitMap(adUnitCodes, bidLimit); // YMPB
        ...
        if (Object.hasOwnProperty.call(targetingSet, adUnitCode)) {
          ...
          if (hbPb > 0) {
            ...
            if (hbPb < getYmpbOption('YMPB_CPM_TARGET_MIN')) {
              ...
              targetingSet[adUnitCode].hb_pb = getCpmStringValue(hbPb, config.getConfig('customPriceBucket'), 1);
              debugger;
            }
          }
        }
