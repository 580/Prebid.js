...
export function newAuctionManager() {
  ...
  /**
   * Prebid will clear the auction in v8.
   * This function is no longer required.
   */
  auctionManager.clearAuctions = function() { // YMPB
    let _now = Date.now();
    let _currentAuctions = _auctions.toArray();

    for (let index = _currentAuctions.length - 1; index >= 0; index--) {
      let auction = _currentAuctions[index];
