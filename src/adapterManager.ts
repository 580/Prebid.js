...
import { checkBidderTimeout } from './ympb.js'; // YMPB
...
const adapterManager = {
  ...
  makeBidRequests: hook('sync', function (
    ...
    adUnits.forEach(au => {
      ...
      au.bids = au.bids.filter((bid) => {
        ...
        if (checkBidderTimeout(au.code, bid)) {
          return false;
        }
  ...
  checkBidAdapter: function(bidderCode: string): boolean {
    // let s2sConfig = config.getConfig('s2sConfig');
    // let s2sBidders = s2sConfig && s2sConfig.bidders;
    return typeof _bidderRegistry[bidderCode] !== 'undefined'; // || (s2sBidders && includes(s2sBidders, bidderCode));
  }
