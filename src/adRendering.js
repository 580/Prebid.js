...
export function handleNativeMessage(data, bidResponse, {resizeFn, fireTrackers = fireNativeTrackers}) {
  switch (data.action) {
    case 'resizeNativeHeight':
      // YMPB: get slot area size
      let slotSize = getSlotAreaSize(bidResponse.adUnitCode);
      resizeFn(Math.max(data.width, slotSize[0]), Math.max(data.height, slotSize[1]));
...
// YMPB
function getSlotAreaSize(adUnitCode) {
  let slotSize = [0, 0];
  
  try {
    if (YMPB.getMaxSizeByUnitCode) {
      slotSize = YMPB.getMaxSizeByUnitCode(adUnitCode);
    } else {
